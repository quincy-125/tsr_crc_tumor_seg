...
def run_subprocess_commands(command_list):
    ...
    """
    try:
        p = subprocess.run(command_list, capture_output=True, check=True, text=True)
...
--output_anomaly_polygons_bigquery={output_anomaly_polygons_bigquery} \\
--output_nuclei_segmentation_cell_coords_bigquery={output_nuclei_segmentation_cell_coords_bigquery} \\
--output_nuclei_segmentation_cell_coords_json={output_nuclei_segmentation_cell_coords_json} \\
--output_patch_coordinates={output_patch_coordinates} \\
--output_anomaly_polygon_geoson={output_anomaly_polygon_geoson} \\
--output_anomaly_polygon_visualizations={output_anomaly_polygon_visualizations} \\
--output_anomaly_polygon_debugging_pickle={output_anomaly_polygon_debugging_pickle} \\
--slide_name={slide_name} \\
--png_patch_stitch_gcs_glob_pattern={png_patch_stitch_gcs_glob_pattern} \\
--wsi_stitch_gcs_path={wsi_stitch_gcs_path} {wsi_slide_type} \\
--target_image_width={target_image_width} \\
--patch_height={patch_height} \\
--patch_width={patch_width} \\
--patch_depth={patch_depth} \\
--thumbnail_method={thumbnail_method} \\
--polygons_initially_drop_below={polygons_initially_drop_below} \\
...
""".format(
        ...
        stitch_query_mahalanobis_distance_images_linear=(
            config["output"]["output_query_mahalanobis_distance_images_linear"]
        ),
        ...
        output_anomaly_polygons_bigquery=(
            config["output"]["output_anomaly_polygons_bigquery"]
        ),
        output_nuclei_segmentation_cell_coords_bigquery=(
            config["output"]["output_nuclei_segmentation_cell_coords_bigquery"]
        ),
        output_nuclei_segmentation_cell_coords_json=(
            config["output"]["output_nuclei_segmentation_cell_coords_json"]
        ),
        output_patch_coordinates=config["output"]["output_patch_coordinates"],
        output_anomaly_polygon_geoson=config["output"]["output_anomaly_polygon_geoson"],
        output_anomaly_polygon_visualizations=config["output"][
            "output_anomaly_polygon_visualizations"
        ],
        output_anomaly_polygon_debugging_pickle=config["output"][
        ...
        wsi_slide_type=f'--wsi_slide_type={config["input"]["wsi_slide_type"]}'
        if config["input"]["wsi_slide_type"] is not None
        else "",
        ...
        polygons_initially_drop_below=config["inference"]["gan"][
            "polygons_initially_drop_below"
        ],
        rgb2hed_threshold=config["inference"]["gan"]["rgb2hed_threshold"],
        include_patch_threshold=(config["inference"]["gan"]["include_patch_threshold"]),
        batch_size=config["inference"]["batch_size"],
        gan_export_dir=config["inference"]["gan"]["gan_export_dir"],
        gan_export_name=config["inference"]["gan"]["gan_export_name"],
        generator_architecture=(config["inference"]["gan"]["generator_architecture"]),
        berg_use_Z_inputs=(config["inference"]["gan"]["berg_use_Z_inputs"]),
        berg_latent_size=(config["inference"]["gan"]["berg_latent_size"]),
        berg_latent_mean=(config["inference"]["gan"]["berg_latent_mean"]),
        berg_latent_stddev=(config["inference"]["gan"]["berg_latent_stddev"]),
        ...
        min_neighborhood_count=(config["inference"]["gan"]["min_neighborhood_count"]),
        ...
        segmentation_stride=config["inference"]["segmentation"]["segmentation_stride"],
    ...
    assert os.path.isfile(shell_file) and os.access(
        shell_file, os.R_OK
...
def get_patch_coords_from_gcs(gcs_path):
    ...
    return [(int(line.split(",")[0]), int(line.split(",")[1])) for line in data_lines]


def get_beam_outputs(
    output_gcs_path, gan_output_types_set, segmentation_output_types_set, gs_image_set
):
    """Calls beam stitch pipeline.
    ...
    output_dict["patch_coordinates"] = get_patch_coords_from_gcs(
        gcs_path="{}_{}".format(output_gcs_path, "patch_coordinates.csv-00000-of-00001")
    )
    return output_dict
