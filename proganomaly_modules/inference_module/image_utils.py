...
def multipoly_to_json(geometry, json_file_name):
    """Store Shapely geometry contours into GeoJson File.

    Args:
        geometry: Shapely geometry, could be a `MultiPolygon`,
            `GeometryCollection`, `Polygon`, or `LineString`.
        json_file_name: str, name of multi-polygon json file to save to disk. Json file extensions must 
        not be provided. If blank, then doesn't save.
    """
    multipoly_json_obj = shapely.geometry.mapping(geometry)
    
    json_file_extension = ".json-00000-00001"
    
    condition = json_file_extension in json_file_name
    assert condition==False, "Check your input json file name, remove json file extension from your input"
    
    json_file_full_name = json_file_name + json_file_extension
    
    if json_file_name:
        with open(json_file_full_name, "w") as f:
            geojson.dump(multipoly_json_obj, f)
    return multipoly_json_obj


def json_to_multipoly(json_file_name):
    """Convert GeoJson Object into Shapely geometry contours.
    ...
    with open(json_file_full_name, "r") as f:
        multipoly_json_obj = geojson.load(f)
    
    geometry = shapely.geometry.asShape(multipoly_json_obj)
    return geometry