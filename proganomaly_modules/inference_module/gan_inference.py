...
def plot_predictions(predictions, num_rows, params):
    ...
    if params["export_generated_images"] and params["output_generated_images"]:
    ...
    if (
        ...
        and params["output_encoded_generated_logits"]
    ):
        print("Encoded generated logits:")
        encoded_generated_logits = predictions["encoded_generated_logits"]
        print(
            "encoded_generated_logits.shape = {}".format(encoded_generated_logits.shape)
        )
        print(encoded_generated_logits)

    if (
        params["export_encoded_generated_images"]
        and params["output_encoded_generated_images"]
    ):
        print("Encoded generated images:")
        encoded_generated_images = predictions["encoded_generated_images"]
        print(
            "encoded_generated_images.shape = {}".format(encoded_generated_images.shape)
        )

        image_utils.plot_images(
            images=image_utils.descale_images(images=encoded_generated_images),
            depth=params["image_depth"],
            num_rows=num_rows,
        )
    ...
    if params["export_query_encoded_logits"] and params["output_query_encoded_logits"]:
        print("Encoded query logits:")
        query_encoded_logits = predictions["query_encoded_logits"]
        print("query_encoded_logits.shape = {}".format(query_encoded_logits.shape))
        print(query_encoded_logits)

    if params["export_query_encoded_images"] and params["output_query_encoded_images"]:
        print("Encoded query images:")
        query_encoded_images = predictions["query_encoded_images"]
        print("query_encoded_images.shape = {}".format(query_encoded_images.shape))
    ...
    if (
        ...
        and params["output_query_gen_encoded_logits"]
    ):
        print("Generator encoded query logits:")
        query_gen_encoded_logits = predictions["query_gen_encoded_logits"]
        print(
            "query_gen_encoded_logits.shape = {}".format(query_gen_encoded_logits.shape)
        )
        print(query_gen_encoded_logits)

    if (
        params["export_query_gen_encoded_images"]
        and params["output_query_gen_encoded_images"]
    ):
        print("Generator encoded query images:")
        query_gen_encoded_images = predictions["query_gen_encoded_images"]
        print(
            "query_gen_encoded_images.shape = {}".format(query_gen_encoded_images.shape)
        )

        image_utils.plot_images(
            images=image_utils.descale_images(images=query_gen_encoded_images),
            depth=params["image_depth"],
            num_rows=num_rows,
        )
    ...
    if (
        ...
        and params["output_query_enc_encoded_logits"]
    ):
        print("Encoder encoded query logits:")
        query_enc_encoded_logits = predictions["query_enc_encoded_logits"]
        print(
            "query_enc_encoded_logits.shape = {}".format(query_enc_encoded_logits.shape)
        )
        print(query_enc_encoded_logits)

    if (
        params["export_query_enc_encoded_images"]
        and params["output_query_enc_encoded_images"]
    ):
        print("Encoder encoded query images:")
        query_enc_encoded_images = predictions["query_enc_encoded_images"]
        print(
            "query_enc_encoded_images.shape = {}".format(query_enc_encoded_images.shape)
        )

        image_utils.plot_images(
            images=image_utils.descale_images(images=query_enc_encoded_images),
            depth=params["image_depth"],
            num_rows=num_rows,
        )
    ...
    if (
        ...
        and params["output_query_anomaly_images_sigmoid"]
    ):
        query_anomaly_images_sigmoid = predictions["query_anomaly_images_sigmoid"]
        if params["image_depth"] > 1:
            ...
            image_utils.plot_images(
                images=image_utils.descale_images(images=query_anomaly_images_sigmoid),
                depth=params["image_depth"],
                num_rows=num_rows,
            )
            ...
            greyscale_query_anomaly_images_sigmoid = query_anomaly_images_sigmoid
    ...
    if (
        ...
        and params["output_query_anomaly_images_linear"]
    ):
        query_anomaly_images_linear = predictions["query_anomaly_images_linear"]
        if params["image_depth"] > 1:
            ...
            image_utils.plot_images(
                images=image_utils.descale_images(images=query_anomaly_images_linear),
                depth=params["image_depth"],
                num_rows=num_rows,
            )
    ...
    if (
        ...
        and params["output_query_mahalanobis_distances"]
    ):
        print("Query Mahalanobis distances:")
        query_mahalanobis_distances = predictions["query_mahalanobis_distances"]
        print(
    ...
    if (
        ...
        and params["output_query_mahalanobis_distance_images_sigmoid"]
    ):
        print("Query Mahalanobis distance images sigmoid:")
        query_mahalanobis_distance_images_sigmoid = predictions[
    ...
    if (
        ...
        and params["output_query_mahalanobis_distance_images_linear"]
    ):
        print("Query Mahalanobis distance images linear:")
        query_mahalanobis_distance_images_linear = predictions[
    ...
    if (
        ...
        and params["output_query_pixel_anomaly_flag_images"]
    ):
        print("Query pixel anomaly flag images:")
        query_pixel_anomaly_flag_images = predictions["query_pixel_anomaly_flag_images"]
        print(
        ...
        image_utils.plot_images(
            images=image_utils.descale_images(images=query_pixel_anomaly_flag_images),
            depth=1,
            num_rows=num_rows,
        )
    ...
    if (
        ...
        and params["output_query_pixel_anomaly_flag_counts"]
    ):
        print("Query pixel anomaly flag counts:")
        query_pixel_anomaly_flag_counts = predictions["query_pixel_anomaly_flag_counts"]
        print(
    ...
    if (
        ...
        and params["output_query_pixel_anomaly_flag_percentages"]
    ):
        print("Query pixel anomaly flag percentages:")
        query_pixel_anomaly_flag_percentages = predictions[
    ...
    if all(
        ...
        print("query_anomaly_scores.shape = {}".format(query_anomaly_scores.shape))
    ...
    if all(
        ...
        print("query_anomaly_flags.shape = {}".format(query_anomaly_flags.shape))
...
def plot_all_exports(
    ...
    if exports_on_gcs:
        ...
        export_list = sorted(os.listdir(os.path.join(params["output_dir"], "export")))
    print(
        "There are {} exported models at path {}".format(
            len(export_list), os.path.join(params["output_dir"], "export")
        )
    )
    print(export_list)
    ...
    for i in range(export_start_idx, export_end_idx):
        ...
        if params["export_all_growth_phases"]:
            ...
            predictions_by_growth = get_predictions.get_current_growth_predictions(
                export_name=export,
                Z=Z,
                query_images=image_utils.scale_images(query_images),
                params=params,
        ...
        for i, growth in enumerate(predictions_by_growth):
            if i in only_output_growth_set:
                plot_predictions(predictions=growth, num_rows=num_rows, params=params)
...
def inference_from_saved_model(query_images, output_types_set, gs_image_set, config):
    """Inferences from SavedModel.
    ...
    """
    image_dict = {}
    if "query_images" in output_types_set and len(output_types_set) == 1:
        images = image_utils.scale_images(images=query_images)
        image_dict = {"query_images": images}
    else:
        Z = None
        if config["generator_architecture"] == "berg" and config["berg_use_Z_inputs"]:
            Z = tf.random.normal(
                shape=(query_images.shape[0], config["berg_latent_size"]),
        ...
        # berg.
        if (
            "generated_images" in output_types_set
            and config["generator_architecture"] == "berg"
        ):
            image_dict["generated_images"] = predictions_by_growth[-1][
                "generated_images"
            ]
        if (
            "encoded_generated_images" in output_types_set
            and config["generator_architecture"] == "berg"
        ):
        ...
        if (
            "query_encoded_images" in output_types_set
            and config["generator_architecture"] == "berg"
        ):
        ...
        # GANomaly.
        if (
            "query_gen_encoded_images" in output_types_set
            and config["generator_architecture"] == "GANomaly"
        ):
        ...
        if any(
            ...
            image = predictions_by_growth[-1]["query_anomaly_images_linear"]
        ...
        if "query_mahalanobis_distance_images_linear" in output_types_set:
            image_dict[
                "query_mahalanobis_distance_images_linear"
            ] = predictions_by_growth[-1]["query_mahalanobis_distance_images_linear"]
        # berg and GANomaly.
        if any(
            [
            ...
            if config["custom_mahalanobis_distance_threshold"] >= 0.0:
                ...
                image = tf.where(
                    condition=tf.greater(
                        x=distances, y=config["custom_mahalanobis_distance_threshold"]
                    ),
                ...
                # shape = (batch, height, width)
                image = predictions_by_growth[-1]["query_pixel_anomaly_flag_images"]

            if "query_pixel_anomaly_flag_images" in output_types_set:
                # shape = (batch, height, width, 1)
                image_dict["query_pixel_anomaly_flag_images"] = tf.expand_dims(
                    # shape = (batch, height, width)
                    input=image,
                    axis=-1,
                )
        ...
        image_dict = {k: np.rot90(m=v, k=2, axes=(0, 1)) for k, v in image_dict.items()}
    return image_dict
