...
def get_wsi_thumbnail(wsi, target_image_width):
    """Gets thumbnail from openslide WSI.

    Args:
        wsi: openslide.OpenSlide, An OpenSlide object of the Whole Slide Image.
    ...
    logging.info("Creating thumbnail of slide...")
    thumbnail = wsi.get_thumbnail(size=(patch_size_x, patch_size_y))
    logging.info("Slide thumbnail created.")
    return thumbnail
...
def create_full_slide_mask(binary_img, slide_dims):
    ...
    binary_img_image = tf.reshape(
        tensor=binary_img, shape=(1, binary_img.shape[0], binary_img.shape[1], 1)
    )
    return tf.image.resize(
        images=binary_img_image, size=(slide_dims[1], slide_dims[0]), method="nearest"
    )[0, :, :, 0]
...
def wsi_build_grid(
    ...
    Returns:
        List of dictionaries containing batch index, 4-ary tree indices and
            ...
            in collection.
    """
    grid_list_of_lists_of_dicts = [[{} for j in range(max_dim)] for i in range(max_dim)]

    # We round up below in order not to "throw away" any tissue or other part
    # of the slide. Rounding down (using
    # `block_height = math.ceil(slide_dims[1] / patch_height)`, and similarly
    # for `block_width`, was causing pipeline outputs to be difficult or
    # impossible to "re-align" with the original slide image. Rounding up here
    # solves that issue.
    block_height = math.ceil(slide_dims[1] / patch_height)
    block_width = math.ceil(slide_dims[0] / patch_width)
    patches_added = 0
    for i in range(block_height):
        ...
        for j in range(block_width):
            ...
            if percent > include_patch_threshold:
                grid_list_of_lists_of_dicts[i][j]["coords"] = (low_height, low_width)
    ...
    blocks = [[((0, height), (0, width)) for _ in range(width)] for _ in range(height)]
    ...
    for h in range(height):
        for w in range(width):
            ...
            grid_list_of_lists_of_dicts[h][w]["grid_global_idx_stack"] = indices[h][w][
                :-1
            ]
