...
class RestoredModel(object):
    ...
    def run_sess(self, patches):
        ...
        generated_mask = self.sess.run(fetches=[self.c_mask_out], feed_dict=feed_dict)
...
def image2patch(
    in_image, patch_size, stride, median_blur_image=False, median_blur_kernel_size=9
):
    ...
    if len(shape) > 2:
        full_image = np.pad(in_image, ((0, hpad), (0, wpad), (0, 0)), mode="symmetric")
    else:
...
def draw_individual_edge(mask):
    ...
    for index in np.unique(individual_mask):
        ...
        temp_mask = skimage.morphology.dilation(temp_mask, skimage.morphology.square(3))
...
def cell_seg_main(query_images, config):
    ...
    restored_model = RestoredModel(
        model_name=os.path.join(
            config["segmentation_export_dir"], config["segmentation_model_name"]
        ),
...
def mask_to_polygons(mask, min_area=10.0):
...
def get_cell_polygon_coords(query_images, config):
    ...
    for grayscale_map in grayscale_maps:
        ...
        nuclei_coords_list.append(
            [list(polygon.centroid.coords)[0] for polygon in cell_polygons]
        )
...
def inference_from_saved_model(patch_coords_list, query_images, config):
    ...
    """
    cell_coords_list, nuclei_coords_list = get_cell_polygon_coords(query_images, config)
    cell_coords_dict_list = []
    nuclei_coords_dict_list = []
    for i, coords in enumerate(patch_coords_list):
        cell_coords_dict_list.append({str(coords): cell_coords_list[i]})
        nuclei_coords_dict_list.append({str(coords): nuclei_coords_list[i]})
