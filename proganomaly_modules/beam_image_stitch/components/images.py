...
class BranchCombineDoFn(beam.DoFn):
    ...
    def merge_images_into_square(self, top_left, top_right, bottom_left, bottom_right):
        """Combines leaf images of 4-ary tree.
    ...
    def descale_images(self, images):
        ...
        Returns:
            Tensor of images with range [0, 255] of shape
                (num_images, height, width, num_channels).
        """
        image = tf.cast(x=((images + 1.0) * (255.0 / 2.0)), dtype=tf.uint8)

        image = tf.where(
            condition=image < 0, x=tf.zeros_like(input=image, dtype=tf.uint8), y=image
        )
        image = tf.where(
            condition=image > 255,
