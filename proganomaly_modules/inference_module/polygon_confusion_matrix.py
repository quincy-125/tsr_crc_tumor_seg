...
from proganomaly_modules.beam_polygon_confusion_matrix.components import (
    confusion_matrix,
)
...
def save_polygon_confusion_matrices(config):
    ...
    if config["output"]["use_dataflow"]:
        ...
        polygon_obj = polygon.LocalPolygonDoFn(
            ...
            annotations_image_gcs_path=(config["input"]["annotations_image_gcs_path"]),
            # TODO: Update this now that KDE functionality is being removed:
            kde_gs_image_gcs_path=config["input"]["kde_gs_image_gcs_path"],
            patch_coordinates_gcs_path=(config["input"]["patch_coordinates_gcs_path"]),
        ...
        for threshold in config["polygon"]["thresholds"]:
            ...
            for dilation in config["polygon"]["dilation_factors"]:
                print("threshold = {}, dilation = {}".format(threshold, dilation))
                confusion_matrix_dict = polygon_obj.get_polygon_confusion_matrix_dict(
