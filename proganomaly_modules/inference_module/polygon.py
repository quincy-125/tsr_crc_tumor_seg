...
def create_patch_polygon(
    patch_coords, patch_height, patch_width, height_scale_factor, width_scale_factor
):
    ...
    patch_width, patch_height = (
        patch_width * width_scale_factor,
        patch_height * height_scale_factor,
    )
...
def process_image(image):
    ...
    """
    return (
        tf.cast(x=255 - image_utils.descale_images(image), dtype=tf.float32).numpy()
        / 255.0
    )
...
def get_image_bounds(query_image):
    ...
    Returns:
        Processed image tensor in range [0., 1.].
    """
    image_max_across_channels = tf.math.reduce_max(input_tensor=query_image, axis=-1)
...
def create_original_multipolygon(image):
    ...
    contours = skimage.measure.find_contours(
        image, level=0.5, fully_connected="low", positive_orientation="low", mask=None
    )
...
def clip_polygon_vertices(polygon, x_bounds, y_bounds):
    ...
    fixed_vertices = [clip_vertex(vertex, x_bounds, y_bounds) for vertex in vertices]
    return shapely.geometry.Polygon(
